...
const LoginScreen: React.FC<Props> = ({ onLogin }) => {
  ...
  let dablTiles = [
    ...
    <Tile key="parties">
      <PartiesLoginForm
        onLogin={onLogin}
        setUploadedParties={setUploadedParties}
      />
    </Tile>,
    <Tile key="jwt">
  ...
  if (uploadedParties.length != 0) {
    dablTiles.splice(
      ...
      0,
      <Tile>
        <Button className="ghost dark" onClick={() => setShowQuickSetup(true)}>
          Quick Setup
        </Button>
      </Tile>
    );
...
const PartiesLoginForm: React.FC<PartiesLoginFormProps> = ({
  onLogin,
  setUploadedParties,
}) => {
  ...
  const handleLogin = async (party?: PartyDetails) => {
    ...
    if (partyDetails) {
      ...
      throw new AppError(
        "Failed to Login",
        "No parties.json or party selected"
      );
    }
  };
  ...
  return (
    <>
      <p className="login-details dark">
        <span>
          Alternatively, login with <code className="link">parties.json</code>{" "}
          located in the DABL Console Users tab:{" "}
        </span>
      </p>
      <FormErrorHandled
        size="large"
