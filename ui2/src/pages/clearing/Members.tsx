  ...
  return (
    <div className='assets'>
      ...
      <StripedTable
        ...
        rows={
          services.map(s => {
            const standing = standings.find(standing => standing.payload.customer === s.payload.customer);
            const clearingDeposits = deposits.filter(d => d.payload.account.id.label == s.payload.clearingAccount.id.label);
            const marginDeposits = deposits.filter(d => d.payload.account.id.label == s.payload.marginAccount.id.label);
            ...
            return [
                ...
                marginAmount,
                standingText,
                <IconButton color="primary" size="small" component="span" onClick={() => history.push("/app/clearing/member/" + s.contractId.replace("#", "_"))}>
                  <KeyboardArrowRight fontSize="small" />
                </IconButton>
              ];
          })
