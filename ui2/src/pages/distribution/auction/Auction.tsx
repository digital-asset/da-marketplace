          ...
          <Table basic='very'>
            <Table.Body>
              ...
              <Table.Row key={3}>
                ...
                <Table.Cell key={1}>{auction.payload.asset.quantity} {auction.payload.asset.id.label}</Table.Cell>
              </Table.Row>
              <Table.Row key={4}>
                <Table.Cell key={0}><b>Floor</b></Table.Cell>
                <Table.Cell key={1}>{auction.payload.floorPrice} {auction.payload.quotedAssetId.label}</Table.Cell>
              </Table.Row>
              <Table.Row key={5}>
              ...
              {getFinalPrice(auction.payload.status)
                ...
                <Table.Row key={7} className={classes.tableRow}>
                  <Table.Cell key={0}><b>Final price</b></Table.Cell>
                  <Table.Cell key={1}>{getFinalPrice(auction.payload.status)} {auction.payload.quotedAssetId.label}</Table.Cell>
                </Table.Row>
                :
                <Table.Row key={8} className={classes.tableRow}>
                  <Table.Cell key={0}><b>Current price</b></Table.Cell>
                  <Table.Cell key={1}>{currentPrice.toFixed(2)} {auction.payload.quotedAssetId.label}</Table.Cell>
                </Table.Row>
              }
              {getParticallyAllocatedUnits(auction.payload) &&
                <Table.Row key={9} className={classes.tableRow}>
                  <Table.Cell key={0}><b>Allocated</b></Table.Cell>
                  <Table.Cell key={1}>{getParticallyAllocatedUnits(auction.payload)?.toFixed(2)} {auction.payload.asset.id.label}</Table.Cell>
                </Table.Row>
              }
              <Button type='submit' className='ghost details-button' disabled={auction.payload.status.tag !== "Open" || bids.length === 0} onClick={closeAuction}>Close Auction</Button>
            </Table.Body>
          </Table>
        </Tile>
            ...
            rows={
              biddingProviderServices
                ...
                .map(c => [
                  ...
                  isAuctionProvider && <Button floated='right' type='submit' className='ghost' disabled={getbiddingAuctionstatus(c.payload.customer) !== "No bid requested" || auction.payload.status.tag !== "Open"} onClick={() => requestBid(c)}>Request Bid</Button>
                ])
            }
          />
        </Tile>
