...
export const getBidAllocation = (bid : Bid) : string => {
  switch (bid.status.tag) {
    case 'PartialAllocation' :
      return bid.status.value.quantity + " " + bid.assetId.label + " @ " + bid.status.value.price + " " + bid.quotedAssetId.label
    case 'FullAllocation' :
      return bid.details.quantity + " " + bid.assetId.label + " @ " + bid.status.value.price + " " + bid.quotedAssetId.label
    default:
      return ""
  }
};
