...
import { ServiceKind, ServiceRequest, ServiceRequestTemplates, useProviderServices  } from '../../context/ServicesContext';
...
const Relationship: React.FC<RelationshipProps> = ({ provider, services }) => (
  <Tile className='relationship-tile'>
    ...
    <div className='child'>{services.map(s => <Label key={s} content={s}/>)}</div>
  </Tile>
)
...
const Landing = () => {
  ...
  const [ requestParams, setRequestParams ] = useState<RequestInterface>({ provider: '', customer: '' });

  useEffect(() => {
    const provider = identities.find(i => i.payload.legalName === dialogState?.provider)?.payload.customer || "";
  ...
  const requestService = <T extends ServiceRequestTemplates,>(service: Template<T, undefined, string>, kind: ServiceKind, extraFields?: object) => {
    setFields({
      provider: {
        label: "Provider",
        type: "selection",
      ...
      <div className='col col-1'>
        ...
        <Tile>
          <div className='link-tile'>
            <div>
              <h3>Portfolio</h3>
              <span className='balance'><h3>{portfolio}</h3></span><span>USD</span>
            </div>
            <div className='link'>
        ...
        <div>
          ...
          <OverflowMenu>
            ...
            <OverflowMenuEntry
              ...
              onClick={() => requestService(TradingRequest, ServiceKind.TRADING, {
                tradingAccount: {
