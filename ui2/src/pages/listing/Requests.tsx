    ...
    <>
      <Grid container direction="column">
        <Grid container direction="row">
          ...
          <Grid item xs={12}>
            <Paper className={classes.paper}>
              <Grid container direction="row" justify="center" className={classes.paperHeading}><Typography variant="h2">Listing Requests</Typography></Grid>
              <Table size="small">
                <TableHead>
                  <TableRow className={classes.tableRow}>
                    ...
                    <TableCell key={3} className={classes.tableCell}><b>Listing ID</b></TableCell>
                    <TableCell key={4} className={classes.tableCell}><b>Calendar ID</b></TableCell>
                    <TableCell key={5} className={classes.tableCell}><b>Traded Asset</b></TableCell>
          ...
          <Grid item xs={12}>
            <Paper className={classes.paper}>
              <Grid container direction="row" justify="center" className={classes.paperHeading}><Typography variant="h2">Delisting Requests</Typography></Grid>
              <Table size="small">
                <TableHead>
                  <TableRow className={classes.tableRow}>
                    <TableCell key={0} className={classes.tableCell}><b>Provider</b></TableCell>
                ...
                <TableBody>
                  {deleteEntries.map((c, i) => (
                    <TableRow key={i} className={classes.tableRow}>
                      ...
                      <TableCell key={5} className={classes.tableCell}>{c.listing?.tradedAssetId.label}</TableCell>
                      <TableCell key={6} className={classes.tableCell}>{c.listing?.quotedAssetId.label}</TableCell>
                      <TableCell key={7} className={classes.tableCell}>
                        {party === c.request.payload.provider && <Button color="primary" size="small" className={classes.choiceButton} variant="contained" onClick={() => deleteListing(c.request)}>Process</Button>}
                        {/* {party === c.payload.client && <Button color="primary" size="small" className={classes.choiceButton} variant="contained" onClick={() => cancelRequest(c)}>Cancel</Button>} */}
                      </TableCell>
                      <TableCell key={8} className={classes.tableCell}>
