    ...
    <div className='issuance-requests'>
      <Tile header={<h2>Actions</h2>}>
        <Button
          secondary
          className='ghost'
          onClick={() => history.push("/app/issuance/new")}>New Issuance</Button>
      </Tile>
        ...
        <StripedTable
          ...
          rows={
            createRequests.map(c => [
              ...
              c.payload.quantity,
              <>
                {party === c.payload.provider && <Button secondary className="ghost" onClick={() => createIssuance(c)}>Issue</Button>}
              </>,
              <IconButton
                color="primary"
                size="small"
                component="span"
                onClick={() => history.push("/app/issuance/createrequest/" + c.contractId.replace("#", "_"))}>
        ...
        <StripedTable
          ...
          rows={
            reduceRequests.map(c => [
              ...
              <>
                {party === c.payload.provider && <Button secondary className="ghost" onClick={() => deleteIssuance(c)}>Deissue</Button>}
              </>,
              <IconButton
                color="primary"
                size="small"
                component="span"
                onClick={() => history.push("/app/issuance/deleterequest/" + c.contractId.replace("#", "_"))}>
                <KeyboardArrowRight fontSize="small"/>
              </IconButton>
            ])
          }/>
