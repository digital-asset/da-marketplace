...
const AccountComponent: React.FC<RouteComponentProps & ServicePageProps<Service>> = ({ history, services }: RouteComponentProps & ServicePageProps<Service>) => {
      ...
      <div className='account'>
        ...
        <Tile header={<h2>Actions</h2>}>
          <div className='action-row'>
            <Button className='ghost' onClick={() => requestCredit(account.payload.account.id)}>Deposit</Button>
            <Button className='ghost' onClick={() => requestCloseAccount(account)}>Close</Button>
          </div>
        </Tile>
                  ...
                  accountDeposits.map((c) => [
                    ...
                    <>
                      {party === account.payload.account.owner &&
                        <div className='action-row'>
                          <Button className='ghost' onClick={() => requestWithdrawDeposit(c)}>Withdraw</Button>
                          {relatedAccounts.length > 0 &&
                            <Button className='ghost' onClick={() => requestTransfer(c)}>Transfer</Button>
                          }
                        </div>
                      }
                    </>
