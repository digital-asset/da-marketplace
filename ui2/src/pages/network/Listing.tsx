...
import { Service, Request, Offer } from '@daml.js/da-marketplace/lib/Marketplace/Listing'
...
const ListingComponent : React.FC<RouteComponentProps> = ({ history } : RouteComponentProps) => {
  ...
  const offerService = () => {
    const onClose = async (state : any | null) => {
      ...
      await ledger.exercise(Role.OfferListingService, roles[0].contractId, { client: identity.payload.client });
    };
